...
#define ERASEPIC	1002				   //#. 메세지 타입 : 그림 지우기
...
static ERASEPIC_MSG	 g_erasepicmsg; //#. 그림 지우기 메세지
static DRAWLINE_MSG	 g_drawlinemsg; //#. 선 그리기 메세지
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		g_hBtnSendMsg = GetDlgItem(hDlg, IDC_SENDMSG);
		...
		//#. 추가된 컨트롤 핸들 얻기
		hIsUDP = GetDlgItem(hDlg, IDC_ISUDP);
	...
	case WM_COMMAND:
		...
		case IDC_CONNECT:
			...
			//#. 추가된 명령어
			g_isUDP = SendMessage(hIsUDP, BM_GETCHECK, 0, 0);
		...
		//#. 추가한 case문 (파일 전송)
		case IDC_SENDFILE:
			MessageBox(NULL, "아직 구현하지 않았습니다!.", "알림", MB_ICONERROR);
			//#. GetOpenFileName() 윈도우 API를 활용하여 파일 열기 대화상자 열기
			//#. 전송할 파일 선택
			//#. 선택한 파일을 읽어서 서버에 전송
			return TRUE;
		...
		//#. 추가한 case문 (그림 지우기)
		case IDC_ERASEPIC:
			send(g_sock, (char *)&g_erasepicmsg, BUFSIZE, 0);
			return TRUE;
		case IDCANCEL:
...
DWORD WINAPI ClientMain(LPVOID arg)
{
	...
	//#. 앞으로 이 if 문에 g_IsUDP == FALSE등을 추가하여 UDP 기능 추가 필요
	if(g_isIPv6 == false && g_isUDP == false){	//#. TCP/IPv4
		...
		MessageBox(NULL, "아직 구현하지 않았습니다", "알림", MB_ICONINFORMATION);
	}
...
DWORD WINAPI ReadThread(LPVOID arg)
{
	...
	while(1){
		...
		//#. 추가된 else if 문 (erasepic)
		else if (comm_msg.type == ERASEPIC) {
			erasepic_msg = (ERASEPIC_MSG*)&comm_msg;
			SendMessage(g_hDrawWnd, WM_ERASEIT, 0, 0);
		}
	}
